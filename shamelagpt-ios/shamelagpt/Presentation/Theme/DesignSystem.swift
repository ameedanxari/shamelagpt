...
enum DesignSystem {
    ...
    enum Typography {
        private static func customFont(_ name: String, size: CGFloat, weight: Font.Weight) -> Font {
            // Use FontRegistry to provide a SwiftUI Font. FontRegistry will return a UIFont
            // scaled for Dynamic Type and the requested text style. We map requested
            // sizes to a closest UIFont.TextStyle to preserve scaling behavior.
            ...
            case 11: textStyle = .caption2
            default: textStyle = .body
            }

            let font = FontRegistry.shared.swiftUIFont(forLanguage: nil, textStyle: textStyle, weight: weight)
            AppLogger.font.logDebug("DesignSystem.Typography.customFont requested name=\(name) size=\(size) -> resolved swiftUIFont=\(font)")
            return font
        }
