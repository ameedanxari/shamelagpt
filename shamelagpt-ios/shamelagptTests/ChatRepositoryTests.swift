...
final class ChatRepositoryTests: XCTestCase {
    ...
    func testDeleteNonExistentConversation() async throws {
        // When/Then - Deleting non-existent conversation should not crash
        do {
            try await sut.deleteConversation(id: "non-existent")
            // If no error is thrown, the implementation handles it gracefully
        } catch {
            // If error is thrown, verify it's the expected error type
            XCTAssertTrue(error is CoreDataError, "Should throw CoreDataError if any error")
        }
    }
