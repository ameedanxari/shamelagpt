...
class ChatViewModel(
    ...
    fun loadConversation(conversationId: String?, showHydrationUi: Boolean = true) {
        ...
        ChatFlowDiagnostics.markPhase(
            phaseName = "chat.loadConversation.start",
            conversationId = normalizedConversationId,
            threadId = _uiState.value.threadId,
            detail = "showHydrationUi=$showHydrationUi"
        )
        ...
        messageCollectionJob = viewModelScope.launch {
            try {
                ...
                if (normalizedConversationId != null) {
                    ...
                    ChatFlowDiagnostics.markPhase(
                        phaseName = "chat.loadConversation.fetchRemote",
                        conversationId = normalizedConversationId,
                        threadId = _uiState.value.threadId
                    )
                    ...
                    ChatFlowDiagnostics.markPhase(
                        phaseName = "chat.loadConversation.fetchMetadata",
                        conversationId = normalizedConversationId,
                        threadId = _uiState.value.threadId
                    )
                    ...
                    conversationRepository.getMessagesByConversationId(normalizedConversationId)
                        .collect { messages ->
                            ChatFlowDiagnostics.markPhase(
                                phaseName = "chat.loadConversation.observeMessages",
                                conversationId = normalizedConversationId,
                                threadId = _uiState.value.threadId,
                    ...
                    ChatFlowDiagnostics.markPhase(
                        phaseName = "chat.loadConversation.newConversation",
                        detail = "reset-state"
                    )
                ...
                ChatFlowDiagnostics.markPhase(
                    phaseName = "chat.loadConversation.cancelled",
                    conversationId = normalizedConversationId,
                    threadId = _uiState.value.threadId,
                    detail = e.message
                )
                ...
                ChatFlowDiagnostics.markPhase(
                    phaseName = "chat.loadConversation.failed",
                    conversationId = normalizedConversationId,
                    threadId = _uiState.value.threadId,
    ...
    fun startNewConversationForShare() {
        ...
        ChatFlowDiagnostics.clear(detail = "chat.newConversationForShare")
    }
    ...
    fun sendMessage(text: String) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                ChatFlowDiagnostics.markPhase(
                    phaseName = "chat.send.optimisticUpdate",
                    conversationId = currentState.conversationId,
                    threadId = currentState.threadId
                )
                _uiState.update { state ->
                ...
                ChatFlowDiagnostics.markPhase(
                    phaseName = "chat.send.createStream",
                    conversationId = currentState.conversationId,
                    threadId = currentState.threadId
                )
                ...
                ChatFlowDiagnostics.markPhase(
                    phaseName = "chat.send.streamReady",
                ...
                if (currentState.conversationId == null) {
                    ChatFlowDiagnostics.markPhase(
                        phaseName = "chat.send.loadConversation",
                        conversationId = actualConversationId,
                        threadId = _uiState.value.threadId
                    )
                ...
                stream.collect { event ->
                    eventCount++
                    if (eventCount <= 3 || eventCount % 25 == 0 || event.type == "done") {
                        ChatFlowDiagnostics.markPhase(
                            phaseName = "chat.send.collectEvent",
                            conversationId = actualConversationId,
                            threadId = _uiState.value.threadId,
                            detail = "eventType=${event.type} eventCount=$eventCount assembledLen=${assembledContent.length}"
                        )
                    }
                    when (event.type) {
                        "metadata" -> {
    ...
    fun confirmFactCheck(confirmedText: String) {
        ...
        ChatFlowDiagnostics.markPhase(
            phaseName = "chat.factCheck.start",
            conversationId = _uiState.value.conversationId,
            threadId = _uiState.value.threadId,
            detail = "textLen=${confirmedText.length}"
        )
        ...
        viewModelScope.launch {
            try {
                ...
                ChatFlowDiagnostics.markPhase(
                    phaseName = "chat.factCheck.userSaved",
                    conversationId = actualConversationId,
                    threadId = _uiState.value.threadId
                )
                ...
                while (true) {
                    try {
                        ...
                        stream.collect { event ->
                            ...
                            if (eventCount <= 3 || eventCount % 25 == 0 || event.type == "done") {
                                ChatFlowDiagnostics.markPhase(
                                    phaseName = "chat.factCheck.collectEvent",
                                    conversationId = actualConversationId,
                                    threadId = _uiState.value.threadId,
                                    detail = "eventType=${event.type} eventCount=$eventCount assembledLen=${assembledContent.length}"
                                )
                        ...
                        ChatFlowDiagnostics.clear(detail = "chat.factCheck.complete")
                        break
