...
class OCRManagerTest {
    ...
    private fun createTextResult(
        ...
        val blockMocks = blocks.map { (text, lines) -> createTextBlock(text, lines) }
        val textResult = mockk<Text>()
        every { textResult.text } returns fullText
        every { textResult.textBlocks } returns blockMocks
        return textResult
    }

    private fun createTextBlock(text: String, lines: List<String>): TextBlock {
        val lineMocks = lines.map { lineText ->
            mockk<Line> {
                every { this@mockk.text } returns lineText
            }
        }

        return mockk<TextBlock> {
            every { this@mockk.text } returns text
            every { this@mockk.lines } returns lineMocks
        }
    }
}
